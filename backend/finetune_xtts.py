...
LANGUAGE    = "en"
EPOCHS      = 100
BATCH_SIZE  = 1      # RTX 2060 6GB - must be 1 for FP16 GPT trainer
GRAD_ACCUM  = 16     # effective batch = 16
LR          = 5e-6   # back to original LR with FP16
MAX_AUDIO_LEN = 102400  # ~4.6s at 22050Hz - shorter = less VRAM per sample
...
config = GPTTrainerConfig(
    ...
    test_sentences=[],
    mixed_precision=True,   # FP16 required for 6GB VRAM
    grad_clip=1.0,           # prevent FP16 gradient explosion -> NaN
    use_grad_scaler=True,    # keep AMP scaler but clipped
)
